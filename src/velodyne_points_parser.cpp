...
PARA_TABLE::PARA_TABLE()
{
    ...
    para_base_dir = "/home/william/catkin_ws/src/velodyne_points_publish/parameters/bin/parameters";
}
...
bool VELODYNE_PARSER::init_para()
{
    ...
    if(init_lidar16_para() && init_lidar32_para())
        return true;
    else
        return false;
}
...
bool VELODYNE_PARSER::parse_lidar32_process(u_int8_t *cache)
{
    ...
    u_int8_t *fq = fp + 1;
    u_int16_t val = (*fp)*256 + *fq; // WILLIAM: 2-byte flag?
    ...
    while(val == 0xffee)
    {
        ...
        {
            u_int8_t *pAngL = fp + 100*i + 2; // Azimuth Low
            u_int8_t *pAngH = pAngL + 1; // Azimuth High
            u_int16_t angleH = ((*pAngH)*256 + (*pAngL) + 9000)%36000; 
            u_int8_t *pL = fp + 100*i + 4; // distance Low
            u_int8_t *pH = pL + 1; // distance High
            u_int8_t *pVal = pH + 1; // it is supposed to be calibrated reflectivity 
            ...
            for(int j=0; j<HDL32_BEAM_NUM; j++) // 32
            {
                ...
                if(distance >= HDL32_VALID_RADIUS)
                {
                    ...
                    lidar32_pointcloud[beamInd][idx].myIntensity = *pVal; // WILLIAM
        ...
        for(int angle = 0; angle < HDL32_BEAM_POINTSIZE; angle++)
        {
            if(lidar32_pointcloud[beam][angle].valid)
            {
                fileout << lidar32_pointcloud[beam][angle].x/100.0 << "\t\t"<<
                           ...
                           lidar32_pointcloud[beam][angle].myIntensity << "\t\t"<< // WILLIAM
